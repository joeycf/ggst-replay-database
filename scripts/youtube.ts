...
// (scripts/fetch.ts and scripts/fetch-theater.ts).
//
// Ported from the Tōkon repo unchanged apart from this header: the retry
// ladder, the key handling and the 50-per-call hydration are the same on every
// game, and a second copy of a backoff loop is a second place for it to drift.
// Extracted there when the index intake arrived: it needs the
